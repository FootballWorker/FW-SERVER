...
const listByTeam = async (req, res) => {
  try {
    ...
    }).populate("home","_id name").populate("away","_id name").exec();
...
const listForHome = async (req,res) => {
  try {
    ...
    let matches = await Match.find({country: user.country}).sort('date').limit(5).exec()
    res.json(matches)
  } catch (error) {
    return res.status(400).json({
      error: errorHandler.getErrorMessage(error)
...
const listByAudience = async (req, res) => {
  try {
    let matches = await Match.find({ views : {$gte : 50 } })
      .sort("-date")
      .limit(10)
      .exec();
    res.json(matches);
...
const isAppropriate = async (req,res,next) => {
  try {
    ...
    let match = await Match.findById(req.body.matchId).populate("home","_id").exec()
...
const isVicePresident = async (req,res,next) => {
  try {
    let user = await User.findById(req.auth._id).populate("team","_id name").populate("job","_id name").exec()
    if(!user){
      return res.status(403).json({
